 ...
 *
 */
const requestHttp = (url, api, authtype, authdata) => {
  ...
  instance.interceptors.response.use(
    (res) => {
      process.env.NODE_ENV === "development" && console.log("[RESPONSE_STATUS]", res.status);
      process.env.NODE_ENV === "development" && console.log("[RESPONSE_HEADERS]", res.headers);
      process.env.NODE_ENV === "development" && console.log("[RESPONSE_CONFIG]", res.config);

      return res;
    },
    async (err) => {
      process.env.NODE_ENV === "development" && console.log("[ERR_RESPONSE]", err.response);
